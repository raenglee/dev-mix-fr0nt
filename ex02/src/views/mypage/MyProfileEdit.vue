        ...
        <form @submit.prevent="handleSubmit" class="grid gap-y-6 px-20 py-10">
          ...
          <div class="grid grid-cols-4 items-center gap-x-4">
            ...
            <div class="col-span-2 flex items-center">
              <input type="file" @change="onFileChange" accept="image/*" class="hidden" ref="fileInput"
                placeholder=useStore.proFile.profileImage />
          ...
          <div class="grid grid-cols-4 gap-x-4 items-center">
            <label class="text-gray-700 text-lg font-semibold">기술 / 언어</label>
              ...
              <div class="flex">
                ...
                <div class="flex flex-wrap mt-2">
                  <div v-for="(skill, index) in selectedSkills" :key="index" @click="removeSkill(index)"
                    class="border rounded-full p-2 mr-2 mb-2 flex items-center cursor-pointer">
              ...
              <!--드롭다운-->
              <div v-if="isDropdownOpen" class="absolute bg-white border items-center justify-between border-gray rounded-md z-20 p-4 shadow" style="min-width: 400px">
                <div class="grid grid-cols-4 gap-x-4 items-center">

                  <div v-for="tech in availableTechOptions" :key="tech" @click="selectSkill(tech)"
                    class="p-2 hover:bg-gray-200 cursor-pointer">
                    <img :src="tech.imageUrl" class="flex w-10 h-12 item-center hover:w-12" />
...
const handleSubmit = async () => {
  ...
  const userProfile = {
    email: user.email,
    nickname: nickname.value || user.nickname,  // 변경된 nickname만 사용, 없으면 기존 nickname 사용
    groupName: groupName.value || user.groupName,  // 기존 groupName이 있으면 사용
    location: location.value || user.location,  // 기존 location이 있으면 사용
  ...
  if (profileImage.value) {
    ...
    formData.append('profileImage', file);  // 선택된 파일을 FormData에 추가
