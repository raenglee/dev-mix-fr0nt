      ...
      <div class="my-6 mx-7 justify-center flex flex-col gap-5" style="width: 90%">
        <div v-for="comment in comments" :key="comment.id">
          ...
          <div class="flex items-center mx-2">
            ...
            <router-view v-if="!isPopupOpen"></router-view>
            <p @click="openPopup" class="font-semibold ml-2 text-gray-800">{{ comment.nickname }}</p>
          </div>
...
// 팝업창을 여는 함수
const openPopup = () => {
popupOpen.value = true; // 팝업을 열기 위해 상태를 true로 설정
};

// watchEffect로 팝업 상태를 감지
watchEffect(() => {
if (popupOpen.value) {
// 팝업이 열리면 팝업 창을 띄우고 ESC 키 이벤트를 추가
popupWindow.value = window.open('/userprofile', 'popupWindow', 'width=600,height=800,scrollbars=yes,resizable=yes,toolbar=no,menubar=no,status=no');

// ESC 키를 누르면 팝업을 닫는 이벤트 리스너 추가
const handleKeyDown = (event) => {
  if (event.key === 'Escape' && popupWindow.value) {
    popupWindow.value.close();  // ESC 키가 눌리면 팝업 창을 닫음
    popupOpen.value = false;  // 팝업 상태를 false로 변경
  }
};

// 팝업 창에 이벤트 리스너 추가
if (popupWindow.value) {
  popupWindow.value.addEventListener('keydown', handleKeyDown);
}

// 팝업 창이 닫히면 상태를 false로 변경
popupWindow.value.onbeforeunload = () => {
  popupOpen.value = false;
};
}
// watchEffect에서 팝업이 닫힐 때 이벤트 리스너 제거
if (!popupOpen.value && popupWindow.value) {
popupWindow.value.removeEventListener('keydown', handleKeyDown);
popupWindow.value = null; // 팝업 창 리소스 초기화
}
});
